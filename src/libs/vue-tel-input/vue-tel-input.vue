<template>
  <div :class="['vue-tel-input el-input__inner pl-0', wrapperClasses, { disabled: disabled }]">
    <div
...
<style scoped src="./sprite.css"></style>
<style scoped>
li.last-preferred {
...
import { formatNumber, AsYouType, isValidNumber, parsePhoneNumberFromString } from '../libphonenumber-custom/mobile';
...
export default {
  ...
  mounted() {
    this.initializeCountry()
      .then(() => {
        if (!this.phone && this.showDialCode && this.activeCountry) {
  ...
  methods: {
    ...
    choose(country) {
      ...
      if (this.showDialCode && country) {
        this.phone = `+${country.dialCode}`;
      } else {
        this.phone = '';
      }
      this.$emit('input', this.response.number, this.response);
      //this.dispatch('ElFormItem', 'el.form.blur', [this.phone]);
    },
    testCharacters() {
      const re = /^[()-+0-9\s]*$/;
