  ...
  <div class="reset-pwd-view text-center mt-4">
    ...
    <section v-if="step === 2">
      ...
      <el-form :model="form2" :rules="rules" key="form2" ref="form2">
        ...
        <el-form-item>
          <el-button :loading="validating" @click="submitSMSCaptcha()" class="btn-block" type="primary">下一步</el-button>
        </el-form-item>
      </el-form>
    </section>
    ...
    submitSMSCaptcha() {
      (this.$refs.form2 as ElForm).validate(valid => {
        if (valid) {
          ...
          authService.validateSMS(this.form1.username, this.form2.smsCaptcha)
            .then(() => {
              clearInterval(this.countDownTimer);
